...
<body>
	...
	<div id="center">
		<table class="tableform">
			...
			<tr>
				<td colspan="4">
					<#if project.proj_status == 4>
						<#list devList as d>
							<div class="list" style="color:red;">
								<span class="l">${d.person_name}</span><span class="r"><s>[${d.work_startDate?string("yyyy-MM-dd")}~${d.work_endDate?string("yyyy-MM-dd")}]</s></span>
							</div>
						</#list>
					<#else>
						<#list devList as d>
			...
			<#if project.proj_status == 4>
			<tr>
				<td colspan="4"><b style="color:red;">项目作业时间结束,开发人员将自动退出项目，及如果开发人员的结束日期大于项目结束日期，将重置开发人员的结束日期为项目的结束日期。</b></td>
			</tr>
			</#if>
