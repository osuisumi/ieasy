...
<head>
	...
	$(function() {
		$tg = $("#tg").tree({
			...
			formatter:function(node){
				...
				else if(node.type == "D"){type="<font color='#5CB811'>部门</font>";}
	...
	function savePermits() {
		...
		if(tg) {
			if("O" == tg.type) {
				alertify.warning("无法授权公司级别，请选择部门！");
				return ;
			}
			var principal = {"principalId": tg.id, "principalType": "DEPT"} ;
	...
	function getPermits(node) {
		...
		$.post($.webapp.root+"/admin/system/acl/getPermits.do", {"principalId": node.id, "principalType": "DEPT"}, function(result){
