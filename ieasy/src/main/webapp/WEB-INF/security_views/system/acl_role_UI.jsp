...
<head>
	...
	$(function() {
		$dg = $("#dg").datagrid({
			...
			remoteSort: false, striped:true, pagination:true, pageSize: 30, singleSelect: true,
			sortName: 'created', sortOrder: 'desc',
			columns: [[
			    { field: 'id', title: 'ID', hidden: true },
			    { field: 'name', title: '角色名称', width: 195 }, 
	            ...
	            { field: 'opers', title: '操作', formatter: function(val, row, index){
	            	...
	            	if(undefined != val) {
		            	$.each(val, function(i, p){
		            		op += "<input pid='"+row.id+"' id='"+p.id+"' type='checkbox' operMenuName='"+p.name+"' iconCls='"+p.iconCls+"' sort='"+p.sort+"' href='"+p.href+"'><div class='opts'><label for='"+p.id+"'>"+p.name+"</label></div>" ;
		            	});
	            	}
	            	op + "</div>" ;
	            	return op;
	...
	function savePermits() {
		...
		$.each(dg, function(i, p){
			var principal = {"principalId": p.id, "principalType": "ROLE"} ;
	...
	function getPermits(rowIndex, rowData) {
		...
		$.post($.webapp.root+"/admin/system/acl/getPermits.do", {"principalId": rowData.id, "principalType": "ROLE"}, function(result){
