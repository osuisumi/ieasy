	...
	$(function() {
		...
		$("#import_person_data").euploadify({
            ...
            onUploadSuccess: function(file, data, response) {
            	...
            	if(o.status) {
            		$.easyui.loading({ msg: "正在解析数据，并导入数据库...", locale: "#uploadLayout" }) ;
            		$.post($.webapp.root+'/admin/system/person/doNotNeedAuth_parsePersonExcelData.do', {"datafile": o.savePath}, function(result) {
            			if (result.status) {
            				dg_import.datagrid("loadData", {"rows":result.obj});
            				$.easyui.loaded("#uploadLayout", true);
            			} else {
            				$.messager.alert(result.msg, "error"); $.easyui.loaded("#uploadLayout", true);
            			}
            		}, 'json').error(function() { $.easyui.loaded("#uploadLayout", true); });
            	}
            },
...
<div id="uploadLayout" class="easyui-layout" data-options="fit:true,border:false">
	<div id="uploadNorth" data-options="region:'north', border:false" style="overflow: hidden;">
		<div id="import_person_data"></div>
	</div>
