...
<head>
	...
	$(function() {
		$tg = $("#tg").treegrid({
			url: $.webapp.root + '/static_res/position.tree.json', border: false,
			title: '<span style="font-weight:normal;color:#666;">职位管理</span>',
			...
			columns: [[
	            ...
	            { field: 'name', title: '职位名称', width: 320, sortable: true },
	        ...
	        onAfterEdit: function(data) {
	            ...
	            if(oldSort != data.sort) {
	            	...
	            	$.post($.webapp.root + "/admin/system/position/doNotNeedAuth_sort.do", o, function(result) {
	...
	function form_edit(form) {
		var form_url = $.webapp.root + "/admin/system/position/position_form_UI.do" ;
		if("E" == form) {
			var node = $tg.treegrid('getSelected');
			if (node) {
				form_url = $.webapp.root + "/admin/system/position/position_form_UI.do?id="+node.id ;
			} else {
				alertify.warning("请选择一条记录！"); return ;
			}
		}
		var $d = $.easyui.showDialog({
			href: form_url, title: "表单", iniframe: false, topMost: true, width: 540 , height: 310,
		...
		if(node){
			$.messager.confirm("该操作不可逆，您确定要进行该操作？", function (c) { 
				if(c) {
					...
					$.post($.webapp.root + "/admin/system/position/delete.do", {id:node.id}, function(result) {
						if (result.status) {
							$tg.treegrid('reload') ; $.easyui.loaded();
