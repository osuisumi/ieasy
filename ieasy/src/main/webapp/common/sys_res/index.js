...
(function ($) {
    ...
    window.mainpage.instNavTree = function () {
        ...
        var t = $(navMenuTree);
        t.tree({
        	url: $.webapp.root + "/system/login/getCurrentAuthMenu.do?d="+new Date().getTime(),
            animate: true,
            lines: true,
            toggleOnClick: true,
    ...
    window.mainpage.loadFavoMenus = function () {
        ...
        $(favoMenuTree).tree("load", $.webapp.root+"/common/sys_res/favo-menu-data.json");
    };
    ...
    window.mainpage.bindToolbarButtonEvent = function () {
        ...
        $(btnExit).click(function () {
            $.easyui.messager.confirm("操作提醒", "您确定要退出当前程序？", function (c) {
                if (c) { 
                	$.easyui.loading({ msg: "正在注销，请稍等..." });
                	$.post($.webapp.root+"/system/login/logout.do", null, function(result){
                		if(result.status) {
                			$.easyui.loaded();
                			window.location.replace($.webapp.root+'/login.jsp') ;
                		} else {
                			$.easyui.loaded();
                			alert("安全退出失败，请联系管理员！") ;
                		}
    ...
    window.mainpage.nav.refreshTree = function () {
        if (item && item.id) { window.mainpage.loadMenu(); }
    };
