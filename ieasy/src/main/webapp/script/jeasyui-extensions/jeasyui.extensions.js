...
(function ($, undefined) {
    ...
    var onLoadError = function (XMLHttpRequest, textStatus, errorThrown) {
        ...
        var msg = (XMLHttpRequest && !$.string.isNullOrWhiteSpace(XMLHttpRequest.responseText) ?
                ...
                "错误状态码：" + XMLHttpRequest.status + "(" + XMLHttpRequest.statusText + ")；<hr />" + 
                "" : "");
        var msg_detail = "" ; 
        if(XMLHttpRequest.status === 404) {
        	...
        	msg = "" ; icon = "warning"
        	msg_detail = XMLHttpRequest.responseText ;
        } else if(XMLHttpRequest.status === 205) {
        	msg_detail = "该数据不存在！" ;
        } else {
        	msg_detail = "系统出现了一个未指明的错误，如果该问题重复出现，请联系您的系统管理员并反馈该故障。" ;
        }
        
        var win = $.easyui.messager.alert("提醒", msg+msg_detail, icon),
            opts = win.window("options"), panel = win.window("panel"), width = panel.outerWidth(), height = panel.outerHeight();
        win.window("resize", {width: 400});
        if (width > 800 || height > 800) { win.window("resize", { width: width > 800 ? $(window).width()-400 : width, height: height > 800 ? $(window).height()-50 : height }); }
        win.window("center");
    };
