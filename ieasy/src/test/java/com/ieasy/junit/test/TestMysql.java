...
public class TestMysql {
	...
	public static void backup() {
		try {
			...
			// 调用 调用mysql的安装目录的命令
			Process child = rt.exec("mysqldump -h localhost -uroot -pyhqmcq ieasy");
	...
	public static void load() {
		try {
			...
			Runtime rt = Runtime.getRuntime();
			// 调用 mysql 安装目录的命令
			Process child = rt.exec("mysql -u root -p root dlgs_test");
	...
	public static void load1() {
		try {
			...
			// 调用 mysql 的 cmd:
			Process child = rt.exec("mysql -uroot -pyhqmcq ieasy");
			OutputStream out = child.getOutputStream();// 控制台的输入信息作为输出流
			String inStr;
			StringBuffer sb = new StringBuffer("");
			String outStr;
